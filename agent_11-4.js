...
var initial_LST = function(){
  ...
  setInterval(tick, 500);  // timeslice schedule tick
}
...
modbusClient.on("error", function(error) {
    console.log("SerialPort Error: ", error);
    modbusClient.connectRTUBuffered("\\\\.\\COM1", { baudRate: 9600, parity: "none", dataBits: 8, stopBits: 2 });
});
...
function flow_ctr(){
    ...
    if(flowmeter>0){
        ...
        auto_fan1 = function(){
            modbusClient.setID(4);
            modbusClient.writeRegister(0x0201, 45*10)
            .then(function(){
                Fan1Value = 45;
    	        modbusClient.writeCoil(0x4B, true)
            })
...
function COM_handle(){
	if(modbusClient.isOpen){
			console.log("modbus is open");
	}
	else{
		  console.log("modbus is not open");
			modbusClient.connectRTUBuffered("\\\\.\\COM1", { baudRate: 9600, parity: "none", dataBits: 8, stopBits: 2 })
			.catch(function(e)
        {
            var mbsStatus = e.message;
            console.log(e);
        });
	}
}
...
function tick() {
  ...
  console.log('>>'+counter);
  if (counter%2) {
  	if(auto==true) console.log("manual")
  	if(auto==false&&mud_ctro!=false){
  		mud_ctro()
  		mud_ctro = false	
  	}
  	if(auto==false&&mud_ctrc!=false){
  		mud_ctrc()
  		mud_ctrc=false
  	}
    // if(auto==false){
...
function orp01_read(){
  ...
  modbusClient.readHoldingRegisters(0x04, 2, function(err, data) {
    if(err)
    {
      ...
      orp01 = ieee754.read(cvt, 0, false, 23, 4);//氧化还原电位
   }
  });
}
function orp02_read(){
  ...
  modbusClient.readHoldingRegisters(0x0C, 2, function(err, data) {
    if(err)
    {
      ...
      cvt[2] = data.data[1]>>8;
      cvt[3] = data.data[1]&0x00ff;
      ph = ieee754.read(cvt, 0, false, 23, 4);//ph
   }
  });
}
function orp03_read(){
...
function Fan1Status_R(callback){
  modbusClient.readCoils(0x00, 1, function(err, data) {
    if(err) {
    	console.log("read Fan1status error");
      COM_handle();
    }
    else{
...
function poll_Fan1_t(callback) {
  ...
  Fan1Status_R(function Fan1realValue_R(){
    modbusClient.readHoldingRegisters(0x0220, 1, function(err, data) {
      if(err) {
      	console.log("read Fan1realvalue error");
      	COM_handle();
      }
      else{
